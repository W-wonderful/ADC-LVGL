...
#include "../lv_examples/src/lv_test/test.h"
...
//	LVGL_test();
  ...
  /* Create the mutex(es) */
  /* definition and creation of SYSMutex */
  osMutexDef(SYSMutex);
  SYSMutexHandle = osMutexCreate(osMutex(SYSMutex));
  ...
  osThreadDef(Periodic, PeriodicTask, osPriorityAboveNormal, 0, 128);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : LCD_EN_Pin PB12 PB14 */
  GPIO_InitStruct.Pin = LCD_EN_Pin|GPIO_PIN_12|GPIO_PIN_14;
...
//    HAL_GPIO_TogglePin(GPIOA,Signal2_Pin);
